<template>
  <div>
      ...
      <tbody>
        <template v-for="(archive, index) in archives">
          <tr class="cursor-pointer" :class="tdColoring(archive)" :key="archive._id" @click.prevent="onClickLink(archive, index)">
            <td align="center"><span class="badge block-badge" v-bind:class=ARCHIVES_TYPES[archive.type].label>
    ...
    <b-pagination align="center" size="md" v-model="currentPage" :limit="limit" :total-rows="totalItems" :per-page="limit" @change="getEvents(... arguments)" />
  </div>
</template>
